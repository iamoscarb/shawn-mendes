import type { AlbumInfo } from "../../interfaces/music.interface";

export const ShawnData: AlbumInfo[] = [
    { //Shawn - The Album
        albumName: 'Shawn',
        albumSinger: 'Shawn Mendes',
        albumKey: 'shawnAlbum',
        albumCover: 'https://m.media-amazon.com/images/I/71ewAN10xmL._UF1000,1000_QL80_.jpg',
        bannerImages: {
            desktopBanner: '../banners/shawn-album-hero-desktop.webp',
            mobilBanner: 'https://images.genius.com/5fd36429abce8d036b835539485ed9f4.737x737x1.jpg'
        },
        albumTracks: [
            { name: "Who I Am", singer: 'Shawn Mendes', time: "1:00", track: 1 },
            { name: "Why Why Why", singer: 'Shawn Mendes', time: "1:00", track: 2, url: 'lS8Ps16Eelg?si=RcL5g2wtZ8ssYLTD' },
            { name: "That's The Dream", singer: 'Shawn Mendes', time: "1:00", track: 3 },
            { name: "Nobody Knows", singer: 'Shawn Mendes', time: "1:00", track: 4 },
            { name: "Isn't That Enough", singer: 'Shawn Mendes', time: "1:00", track: 5 },
            { name: "Heart of Gold", singer: 'Shawn Mendes', time: "1:00", track: 6 },
            { name: "Heavy", singer: 'Shawn Mendes', time: "1:00", track: 7 },
            { name: "That'll Be The Day", singer: 'Shawn Mendes', time: "1:00", track: 8 },
            { name: "In Between", singer: 'Shawn Mendes', time: "1:00", track: 9 },
            { name: "The Mountain", singer: 'Shawn Mendes', time: "1:00", track: 10 },
            { name: "Rollin' Right Along", singer: 'Shawn Mendes', time: "1:00", track: 11 },
            { name: "Hallelujah", singer: 'Shawn Mendes', time: "1:00", track: 12 }
        ],
        albumLink: [
            { key: 'spotify', platform: 'Spotify', link: 'https://open.spotify.com/intl-es/album/5FYFgRnlsl9Qrm1xD8cqo7' },
            { key: 'amazonMusic', platform: 'Amazon Music', link: 'https://music.amazon.com.mx/albums/B0DC759W8V' },
            { key: 'appleMusic', platform: 'Apple Music', link: 'https://music.apple.com/es/album/shawn/1761182577' },
            { key: 'otherMusic', platform: 'More', link: 'https://shawn.lnk.to/album' }
        ],
        videosList: [
            { title: 'Shawn Mendes - Why Why Why', img: 'https://is1-ssl.mzstatic.com/image/thumb/Video211/v4/3a/49/e0/3a49e050-a5fe-9a39-d288-7811de3bb6e3/24UMGIM82181.crop.jpg/1040x1040mv.webp' },
            { title: 'Shawn Mendes - Heart of Gold', img: 'https://is1-ssl.mzstatic.com/image/thumb/Video211/v4/30/0e/ee/300eeed5-b28c-b119-69a5-6424474bea7c/24UM1IM05728.crop.jpg/1040x1040mv.webp' },
            { title: 'Shawn Mendes - Nobody Knows', img: 'https://is1-ssl.mzstatic.com/image/thumb/Video221/v4/ab/69/51/ab6951c3-4e12-2ac2-9101-54f3e8da43f0/24UMGIM92910.crop.jpg/1040x1040mv.webp' },
            { title: `Shawn Mendes - Isn't That Enough`, img: 'https://is1-ssl.mzstatic.com/image/thumb/Video221/v4/a1/c7/93/a1c793e4-d738-7034-b89e-da8689fd1913/24UMGIM81920.crop.jpg/1040x1040mv.webp' }
        ],
        photoList: [
            { title: 'shawn 2', img: 'https://i.pinimg.com/1200x/21/80/59/218059cc8b8fc482caac35a4475ddc5d.jpg' },
            { title: 'abx', img: 'https://www.shawnmendesbrasil.com/galeria/albums/userpics/10002/IMG_8994.jpg' },
            { title: 'shirtless shawn', img: 'https://www.shawnmendesbrasil.com/galeria/albums/userpics/10002/IMG_8428~0.JPG' },
            { title: 'shawn', img: 'https://www.shawnmendesbrasil.com/galeria/albums/userpics/10002/IMG_8485.JPG' },
        ]
    },
    { //Wonder - The Album
        albumName: 'Wonder',
        albumSinger: 'Shawn Mendes',
        albumKey: 'wonderAlbum',
        albumCover: 'https://m.media-amazon.com/images/I/71Q9an5gVGS._UF1000,1000_QL80_.jpg',
        bannerImages: {
            desktopBanner: '../banners/wonder-album-hero-desktop.webp',
            mobilBanner: 'https://images.genius.com/5fd36429abce8d036b835539485ed9f4.737x737x1.jpg'
        },
        albumTracks: [
            { name: "Intro", singer: 'Shawn Mendes', time: "1:00", track: 1 },
            { name: "Wonder", singer: 'Shawn Mendes', time: "1:00", track: 2, url: 'lS8Ps16Eelg?si=RcL5g2wtZ8ssYLTD' },
            { name: "Higher", singer: 'Shawn Mendes', time: "1:00", track: 3 },
            { name: "24 Hours", singer: 'Shawn Mendes', time: "1:00", track: 4 },
            { name: "Teach Me How To Love", singer: 'Shawn Mendes', time: "1:00", track: 5 },
            { name: "Call My Friends", singer: 'Shawn Mendes', time: "1:00", track: 6 },
            { name: "Dreams", singer: 'Shawn Mendes', time: "1:00", track: 7 },
            { name: "Song For No One", singer: 'Shawn Mendes', time: "1:00", track: 8 },
            { name: "Monster", singer: 'Shawn Mendes & Justin Bieber', time: "1:00", track: 9 },
            { name: "305", singer: 'Shawn Mendes', time: "1:00", track: 10 },
            { name: "Always Been You", singer: 'Shawn Mendes', time: "1:00", track: 11 },
            { name: "Piece Of You", singer: 'Shawn Mendes', time: "1:00", track: 12 },
            { name: "Look Up At The Stars", singer: 'Shawn Mendes', time: "1:00", track: 13 },
            { name: "Can't Imagine", singer: 'Shawn Mendes', time: "1:00", track: 14 }

        ],
        albumLink: [
            { key: 'spotify', platform: 'Spotify', link: 'https://open.spotify.com/intl-es/album/5FYFgRnlsl9Qrm1xD8cqo7' },
            { key: 'amazonMusic', platform: 'Amazon Music', link: 'https://music.amazon.com.mx/albums/B0DC759W8V' },
            { key: 'appleMusic', platform: 'Apple Music', link: 'https://music.apple.com/es/album/shawn/1761182577' },
            { key: 'otherMusic', platform: 'More', link: 'https://shawn.lnk.to/album' }
        ],
        videosList: [
            { title: 'Shawn Mendes - Intro (Wonder Trailer)', img: 'https://is1-ssl.mzstatic.com/image/thumb/Video114/v4/65/bd/4f/65bd4f59-7621-f4af-7479-072ae027416a/20UMGIM86137.crop.jpg/1040x1040mv.webp' },
            { title: 'Shawn Mendes - Wonder', img: 'https://is1-ssl.mzstatic.com/image/thumb/Video124/v4/aa/22/25/aa2225f0-50b7-ab3b-32ae-27d11498ff22/20UMGIM83365.crop.jpg/1040x1040mv.webp' },
            { title: 'Shawn Mendes & Justin Bieber - Monster', img: 'https://is1-ssl.mzstatic.com/image/thumb/Video124/v4/88/71/fe/8871fe0b-c17f-151a-f97a-e707352d808b/20UM1IM02920.crop.jpg/1040x1040mv.webp' },
        ],
        photoList: [
            { title: 'shawnph', img: 'https://i.pinimg.com/1200x/50/06/44/500644ae4e483caaec522a6ee026ad35.jpg' },
            { title: 'wonder', img: 'https://i.pinimg.com/736x/59/d9/fc/59d9fcc5370e6a867d3aa8e00ea53148.jpg' },
            { title: 'wonder shawn', img: 'https://i.pinimg.com/1200x/b4/01/03/b40103a7d34a98cd940d3259e2f90dc6.jpg' },
            { title: 'shawn', img: 'https://i.pinimg.com/736x/3b/86/e5/3b86e54a2b3e525662edc2e78a321133.jpg' },
        ]
    }
]
